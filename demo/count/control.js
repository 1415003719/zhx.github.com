...
endTime.setTime(endTime.getTime()+3600*1000*2);
var currTime=0;
var balls=[];
...
function updata(){
	...
	if(nextSeconds!=seconds){
		...
		if(parseInt(minutes/10)!=parseInt(nextMinutes/10)){
			addBalls(MARGIN_LEFT+38*(RADIUS+1),MARGIN_TOP,parseInt(minutes/10))
		}
		if(parseInt(minutes%10)!=parseInt(nextMinutes%10)){
			addBalls(MARGIN_LEFT+53*(RADIUS+1),MARGIN_TOP,parseInt(minutes%10))
		}
		if(parseInt(seconds/10)!=parseInt(nextSeconds/10)){
			addBalls(MARGIN_LEFT+76*(RADIUS+1),MARGIN_TOP,parseInt(seconds/10))
		}
		if(parseInt(seconds%10)!=parseInt(nextSeconds%10)){
...
function render(cxt){
	...
	renderDigit(MARGIN_LEFT+38*(RADIUS+1),MARGIN_TOP,parseInt(minutes/10),cxt);
    renderDigit(MARGIN_LEFT+53*(RADIUS+1),MARGIN_TOP,parseInt(minutes%10),cxt);
    renderDigit(MARGIN_LEFT+68*(RADIUS+1),MARGIN_TOP,10,cxt);
    renderDigit(MARGIN_LEFT+76*(RADIUS+1),MARGIN_TOP,parseInt(seconds/10),cxt);
    renderDigit(MARGIN_LEFT+91*(RADIUS+1),MARGIN_TOP,parseInt(seconds%10),cxt);
    for (var i = 0; i < balls.length; i++) {
